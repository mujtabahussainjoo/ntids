...
class CategoryController
{
    ...
    public function getSubCategories(CommonController $common){
		...
		$cinemaCatId = ( isset($_REQUEST['catId']) && $_REQUEST['catId'] != '' ) ? $_REQUEST['catId'] : '43';
		...
		if(trim($cinemaCatId) != '' && trim($accessToken) != '' ){
			...
			if($category){
						...
						if(count($category_dtls->items) > 0){
							...
							if(isset($value->custom_attributes) && count($value->custom_attributes) > 0)
							{
								foreach($value->custom_attributes as $cAttr)
								{
									...
									{
										$tmp['product_image'] = str_replace('appbt/','',$common->store_url()).'pub/media/catalog/category/'.trim(str_replace('pub/media/catalog/category/','',$cAttr->value));
									}
				...
				$json = '{ "success" : "1", "data" : ['.json_encode($subCatArr).'], "message" : "There is no product inside the category."}';
			} else {
				$json = '{ "success" : "0", "data" : [], "message" : "Something went wrong."}';
			}
	...
	public function getCinemasCategories(CommonController $common){
		...
		$category = $common->getCurl($postData,$common->api_url().'categories/'.$cinemaCatId,'GET',$common->admin_token());
	...
	public function getCategoryDetailsWithProducts(CommonController $common){
		...
		if(trim($product_id) != '' && trim($accessToken) != ''){
			...
			if($pCategory)
			{
				...
				$prdArr['prod_s_desc'] = $pCategory->name;
				$prdArr['prod_l_desc'] = $pCategory->name;
				$prdArr['images'] = array();
				foreach($pCategory->custom_attributes as $pAttr)
				{
					if($pAttr->attribute_code == 'url_path')
					...
					{
						$prdArr['images'][] = str_replace('appbt/','',$common->store_url()).'pub/media/catalog/category/'.trim(str_replace('pub/media/catalog/category/','',$pAttr->value));
					}
				}
			...
			if ($type != '') {
				//$types = explode ( ",", $type );
				$condition = '&searchCriteria[filter_groups][0][filters][1][field]=admission&searchCriteria[filter_groups][0][filters][1][value]='.$type.'&searchCriteria[filter_groups][0][filters][1][condition_type]=finset';
			}
			
			$custDetails = $common->getCurl($postData,$common->store_url().'rest/V1/customers/me','GET',$accessToken);
			
			$custOrders = $common->getCurl($postData,$common->api_url().'orders?searchCriteria[filter_groups][0][filters][0][field]=customer_email&searchCriteria[filter_groups][0][filters][0][value]='.$custDetails->email,'GET',$common->admin_token());
