...
class EditTest extends \Magento\TestFramework\TestCase\AbstractBackendController
{
    ...
    public function setUp()
    {
        ...
        $this->uri = 'backend/kbase/category/edit';
        parent::setUp();
    }

    /**
     * @covers  Mirasvit\Kb\Controller\Adminhtml\Category\Edit::execute
     */
    public function testEditAction()
    {
        $this->getRequest()->setParam('id', 4);
        $this->dispatch('backend/kbase/category/edit');
        $body = $this->getResponse()->getBody();
        $this->assertNotEmpty($body);
        $this->assertNotEquals('noroute', $this->getRequest()->getControllerName());
        $this->assertFalse($this->getResponse()->isRedirect());
        $this->assertContains('<h1 class="page-title">Category 1.1</h1>', $body);
    }
}
