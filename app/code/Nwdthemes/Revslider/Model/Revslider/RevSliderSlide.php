...
class RevSliderSlide extends \Nwdthemes\Revslider\Model\Revslider\Framework\RevSliderElementsBase {
			...
			foreach($this->arrChildren as $k => $child){
				if(isset($child->arrLayers) && !empty($child->arrLayers)){
					foreach($child->arrLayers as $key=>$layer){
						...
						$text = RevSliderFunctions::getVal($layer, "text");
						$text = $this->_frameworkHelper->apply_filters('revslider_mod_meta', $text, $postID, $postData); //option to add your own filter here to modify meta to your likings
	...
	private function normalizeParams($params){
		...
		if(isset($params["link"]))
			$params["link"] = RevSliderFunctions::normalizeTextareaContent($params["link"]);
