...
class License extends \Magento\Framework\Model\AbstractModel
{
                ...
                if ($isEnabled && (strpos($mdN, 'WeltPixel_') !== false)
                    ...
                    if ($isLRqd) {
                        ...
                        $licenseModules[$mdN] = [
                            ...
                            "\x6c\x69\x63\x65\x6e\x73\x65" => $lcK,
                            "\x76\x65\x72\x73\x69\x6f\x6e" => $this->getComposerVersion($mdN, \Magento\Framework\Component\ComponentRegistrar::MODULE)
                        ];
                    }
    ...
    public function isLcVd($licNs, $mdN)
    {
        ...
        $matches = [];
        preg_match('/(.local|.dev|.development|.test|.staging|.stage)$/', $domain, $matches);
        if (isset($matches[1])) {
    ...
    protected function _isLRqd($path, &$mdN, $forced = false)
    {
        ...
        try {
            //echo self::LICENCE_KEY_PATH; exit;
            $licNs = $directoryRead->readFile('/var/www/html/app/code/WeltPixel/Backend/etc/module.info');
        } catch (\Exception $ex) {
            return true;
       }
