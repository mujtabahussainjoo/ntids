...
namespace Serole\OrderAdminNotifications\Model\Magento\Sales\Order\Email\Sender;
...
class CreditmemoSender extends \Magento\Sales\Model\Order\Email\Sender\CreditmemoSender
{
    ...
    public function send(Creditmemo $creditmemo, $forceSyncMode = false)
    {
		...
		$billingEmail=$creditmemo->getOrder()->getBillingemail();
        if (!$this->globalConfig->getValue('sales_email/general/async_sending') || $forceSyncMode) {
			...
			$creditmemo->getOrder()->setCustomerEmail('mujtaba.hussain@serole.com');
			if($billingEmail){ 
				$creditmemo->getOrder()->setCustomerEmail($billingEmail);
			}
