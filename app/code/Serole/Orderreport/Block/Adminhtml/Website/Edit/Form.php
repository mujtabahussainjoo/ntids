...
namespace Serole\Orderreport\Block\Adminhtml\Website\Edit;
...
class Form extends Generic
{
    ...
    {
        parent::_construct();
        $this->setId('webiste_form');
        $this->setTitle(__('Website Orders Reports'));
    }
    ...
    protected function _prepareForm()
    {
		...
		// to remove all store for roled based users
		if(count($storeData) < 20)
			 unset($storeData['ALLSTORES']);

        $form = $this->_formFactory->create(
            ['data' => ['id' => 'edit_form', 'action' => $this->getUrl('orderreport/website/post'), 'method' => 'post','enctype' => "multipart/form-data"]]
        );

        $fieldset = $form->addFieldset(
            'base_fieldset',
            ['legend' => __('Website Orders Reports'), 'class' => 'fieldset-wide']
        );
