    ...
    public function execute()
    {
        ...
        try {
			...
			if($isEnabled)
			{
				...
				$isMemberNoValidate = $memberListHelper->isMemberNoValidate(); 
				...
				*/
				...
				$memberCollection->addFieldToFilter('store',array('eq' => $webcode));
				if($isMemberNoRequired && $isMemberNoValidate)
				...
				if($isLastNameRequired)
					$memberCollection->addFieldToFilter('last_name',array('eq' => $lastname));
				...
				//Stores required member no and last name
					...
					if($webcode == 'hbf') {
							...
							if($validMember==false) {
                               ...
                               if(($isMemberNoRequired && $isMemberNoValidate) || $isFirstNameRequired || $isLastNameRequired)
							    {			
				...
				if($isEmailRequired && !empty($requiredEmailFormat) && $requiredEmailFormat !='' && $email != '')
				{
			...
			if(isset($isMemberNoRequired) && $isMemberNoRequired)
			{
			    $customerRepository =  ObjectManager::getInstance()->get('Magento\Customer\Api\CustomerRepositoryInterface');
				$customerData = $customerRepository->getById($customer->getId());
				$customerData->setCustomAttribute("memberno",$memberno);
