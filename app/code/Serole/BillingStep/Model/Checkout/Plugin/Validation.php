...
namespace Serole\BillingStep\Model\Checkout\Plugin;
...
class Validation
{
    ...
    public function beforeSavePaymentInformation(
        ...
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/test-new1.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info($this->isAgreementEnabled());
    ...
    protected function validateAgreements(\Magento\Quote\Api\Data\PaymentInterface $paymentMethod)
    {
        ...
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info($paymentMethod->getExtensionAttributes()->getAgreementIds());
        $logger->info($agreements);
        $logger->info($this->agreementsValidator->isValid($agreements));
