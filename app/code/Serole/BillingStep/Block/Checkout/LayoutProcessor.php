...
namespace Serole\BillingStep\Block\Checkout;
...
class LayoutProcessor extends \Magento\Checkout\Block\Checkout\LayoutProcessor
{
    ...
    public function process($jsLayout)
    {
        ...
        $cartObj = ObjectManager::getInstance()->get(\Magento\Checkout\Model\Cart::class);
        $cartItems = $cartObj->getQuote()->getAllItems();
        $isOnlySimpleProduct = 1;
        foreach($cartItems as $item) {
            $productType = $item->getProductType();
            if($productType == 'virtual'){
                $isOnlySimpleProduct = 0;
            }elseif ($productType == 'grouped'){
                $produdctObj = ObjectManager::getInstance()->create(\Magento\Catalog\Model\Product::class)->load($item->getProductId());
                if($produdctObj->getTypeId() == 'virtual'){
                    $isOnlySimpleProduct = 0;
                }
            }
        }

        if($isOnlySimpleProduct == 0) {
            /*Creating Delivery Email in shipping step*/
            $jsLayout['components']['checkout']['children']['steps']['children']['shipping-step']['children']
            ['shippingAddress']['children']['before-form']['children']['deliveryemail'] = [
                ...
                'dataScope' => 'shippingAddress.deliveryemail',
        ...
        /*$jsLayout['components']['checkout']['children']['steps']['children']['shipping-step']['children']
        ['shippingAddress']['children']['before-form']['children']['confirmdeliveryemail'] = [
            'component' => 'Magento_Ui/js/form/element/abstract',
            'config' => [
                'customScope' => 'shippingAddress',
                'template' => 'ui/form/field',
            ...
            'dataScope' => 'shippingAddress.confirmdeliveryemail',
       ...
       /* $jsLayout['components']['checkout']['children']['steps']['children']['shipping-step']['children']
        ['shippingAddress']['children']['before-from']['children']['confirmdeliveryemail'] = [
            'component' => 'Magento_Ui/js/form/element/abstract',
            'config' => [
                'customScope' => 'shippingAddress',
                'template' => 'ui/form/field',
