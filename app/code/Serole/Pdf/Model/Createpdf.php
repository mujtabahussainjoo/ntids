...
class Createpdf extends \Magento\Framework\Model\AbstractModel {
    ...
    public function __construct(\Magento\Catalog\Model\Product $product,
        ...
        $this->digitalgluedocMissedProductItems = array();
        $this->digitalglueFactory = $digitalglueFactory;
        $this->productsData = array();
    }
    ...
    public function createPdfConcept($orderId,$emailStauts,$reqType){
        ...
        if($order->hasInvoices()) {
		   ...
		   {
			    $logger->info("Folder creation error:".$e->getMessage());
		   }
            ...
            try {
                ...
                foreach ($orderItems as $key => $item) {
                    ...
                    $this->productsData = (array)json_decode($productObj->getProductJsonFormat(), TRUE);
                    ...
                    if ($productObj->getData()) {
                        ...
                        if ($productObj->getTypeId() == "virtual" && $productObj->getData('ni_product_pdf_required') && $item->getParentItemId() == '' && $isSerlized) {
                            ...
                            if ($productObj->getData('digitalglue') == 1){
                                $this->digitalglue['orderId'] = $orderId;
								$digitalGlueSKU = $productObj->getData('digitalglue_sku');
                                $digitalglueArray = array('Quantity' => $qty, 
                                                          'Amount' => $item->getBaseRowTotalInclTax(),
                                                          'SKU' => $digitalGlueSKU, 
                                                          'ReferenceNumber' => $productSku
                                                         );
                                $this->digitalglue['OrderItems'][] = $digitalglueArray;
                ...
                $logger->info($this->isProductMissedJsonData); //digitalgluedocMissedProductItems //$docMissedProductItems

                $digitalGlueResponse = array();
                $digitalGlueResponseDataIssue = 0;
                 if(!empty($this->digitalglue)){
                        $digitalGlue = $this->digitalglueFactory->create();
                        if($reqType == 'frontend'){
                            $digitalGlueResponse = $digitalGlue->getDigitalGlueData($this->digitalglue);
                            if($digitalGlueResponse['status'] == 'success') {
                                $digitalGlueResponse = $digitalGlueResponse['OrderItems'];
                            }else{
                                $digitalGlueResponseDataIssue = 1;
                                $message = $digitalGlueResponse['errorMessage'];
                                $this->errorList['Digitalglue'] = "Digitalglue Data not avilable for this order -".$incrementId;
                                $this->pdfHelper->sendEmailToAdminPDFissue($empty = '', $incrementId, $message);
                            }
                       }else{
                          $digitalGlueColl = $digitalGlue->getCollection();
                          $digitalGlueColl->addFieldToFilter('referenceNumber',$incrementId);
                          $digitalGlueData = $digitalGlueColl->getData();
                          if(!empty($digitalGlueData)) {
                              $digitalGlueResponse = $digitalGlueData;
                          }else{
                              $digitalGlueResponseDataIssue = 1;
                              $this->errorList['Digitalglue'] = "Digitalglue Data not avilable for this order -".$incrementId;
                              $message = "Digitalglue Data not avilable for this order -".$incrementId;
                              $this->pdfHelper->sendEmailToAdminPDFissue($empty = '', $incrementId, $message);
                          }
                       }

                     if (empty($docMissedProductItems) && empty($productNotAvilable) && empty($this->isProductMissedJsonData) && empty($this->errorList)) {
                          $this->digitalGlue($digitalGlueResponse);
                     }
                }
    ...
    public function createOnlyPdf($productSku){
                ...
                foreach ($productAttributeData as $productAttributeItem) {
                    if ($productAttributeItem['type'] == 'image') {
                        ...
                        if ($productAttributeItem['source'] == 'attribute') {
							...
							$logger->info("source:customer");
							$logger->info("customername-data:".$this->customerData['toname']);
                            ...
                            if($productAttributeItem['variable'] == 'customername'){
								...
								$logger->info("customeremail-data:".$this->customerData['email']);
                                $attributeVariableData = $this->customerData['email'];
                                if (!$attributeVariableData) {
                                    $templateProcessor->setValue($productAttributeItem['variable'], ' ');
                                }else{
     ...
     public function digitalGlue($digitalGlueResponse){
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/pdf-createDigitalGlueDoc.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info($digitalGlueResponse);

        try {
             foreach ($digitalGlueResponse as $digitalGlueData) {
                     $productSku = $digitalGlueData['magento_sku'];
                     $productRepository = $this->productRepository;
                     $productObj = $productRepository->get($productSku);
                     ...
                     $docsSaveFilename = "ticket-" . $this->itemData[$productSku]['orderId'] . '-' . trim(str_replace("/","-",$digitalGlueData['receiptnumber'])) . "-document.docx";
                     $pdfFilename = "ticket-" . $this->itemData[$productSku]['orderId'] . '-' . trim(str_replace("/","-",$digitalGlueData['receiptnumber'])) . "-document.pdf";
                     ...
                     foreach ($productAttributeData as $productAttributeItem) {
                         if ($productAttributeItem['type'] == 'image') {
                             ...
                             } elseif ($productAttributeItem['source'] == 'digitalglue') {
                                 $logger->info($productAttributeItem['variable']);
                                 $logger->info("--------------------");
                                 $digitalgluevalue = trim($digitalGlueData[$productAttributeItem['variable']]);
                                 $logger->info($digitalgluevalue);
                                 $attributeVariableData = $digitalgluevalue;
                                 if (!$attributeVariableData) {
                                     $templateProcessor->setValue($productAttributeItem['variable'], ' ');
                                 }else {
                                     $logger->info("Key -".$productAttributeItem['variable']);
                                     $logger->info("Key -".$attributeVariableData);
                                     $templateProcessor->setValue($productAttributeItem['variable'], $attributeVariableData);
                                 }
                             } elseif ($productAttributeItem['source'] == 'customOption') {
                                 /*$itemData is the order Data*/
                             ...
                             if ($productAttributeItem['source'] == 'other') {
                                 ...
                                 $attributeVariableData = $digitalGlueData[$productAttributeItem['variable']];
                                 if (!$attributeVariableData) {
                                     $templateProcessor->setValue($productAttributeItem['variable'], ' ');
                                 }else {
                                     $templateProcessor->setValue($productAttributeItem['variable'], $attributeVariableData);
                             ...
                             if($productAttributeItem['source'] == 'digitalglue') {
                                 $attributeVariableData = $digitalGlueData[$productAttributeItem['variable']];
                                 if (!$attributeVariableData) {
                                     $templateProcessor->setValue($productAttributeItem['variable'], ' ');
                                 }else {
            ...
            $logger->info($e->getMessage());
            $this->createLog("Order-Id $this->orderId- digitalGlue => ".$e->getMessage());
            $status = FALSE;
            $errorMessage = "Order-Id $this->orderId- digitalGlue => ".$e->getMessage();
            $this->pdfGenerationError[] = $e->getMessage();
            $this->pdfHelper->sendEmailToAdminPDFissue($productSku = '', $this->incrementId, $errorMessage);
        }
    }
