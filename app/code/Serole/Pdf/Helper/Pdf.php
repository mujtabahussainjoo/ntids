...
class Pdf extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function sendEmailToAdminPDFissue($productSku,$orderId,$message){
        ...
        $this->_transportBuilder->setTemplateIdentifier($templateId)
            ...
            ->addTo($this->getStoreEmail(), 'Neatideas');
        try {
            $transport = $this->_transportBuilder->getTransport();
            $transport->sendMessage();
            $this->_inlineTranslation->resume();
    ...
    public function sendPdfToCustomerEmail($fileUrl,$data,$orderId){
        ...
        if(isset($data['toname']) && $data['toname']) {
            ...
            //$logger->info($orderData->getData());
            $emailTemplateVariables['toname'] = $data['toname'];
            $emailTemplateVariables['orderid'] = $orderId;
            $emailTemplateVariables['customerName'] = $orderData->getCustomerFirstname().' '.$orderData->getCustomerLastname();
            $emailTemplateVariables['storename'] = $orderData->getStoreName();
			$emailTemplateVariables['customerEmail'] = $orderData->getCustomerEmail();
        }
                            ...
                            if ($productAttributeItem['source'] == 'other') {
                                ...
                                if(isset($this->orderData['product_options']['options'])){
                                    ...
                                    if (is_numeric($key)) {
                                        if(isset($productAttributeItem['duration'])){
                                            $expDate = '';
                                            //$currentDate = date_format($orderItemCustomOptions[$key]['value'],'d-m-Y');
