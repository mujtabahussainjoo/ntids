...
class AppmenuController
{
	...
	public function getMenuOptions(CommonController $common){
		$parentCategoryId = $common->parent_menuid();
		$frgtpwdata = array();		
		...
		$catDetails = $common->getCurl($userData, $common->api_url().'categories/'.$parentCategoryId,'GET',$common->admin_token());
		$newArr = explode(',',$catDetails->children);
		if(is_array($newArr) && !empty($newArr)){
			...
			foreach($newArr as $subCat){
				...
				array_multisort($price, SORT_ASC, $category_data);
			}
			$flag = 0;
			//print_r($category_data); exit;
			foreach($category_data as $subCat){
				$j = $i + 1;
				$result[$i]['title'] = $subCat->name;
				$result[$i]['order'] = $j;
				$result[$i]['id'] = $subCat->id;
				
				//echo $subCat->children.'====='.$subCat->id.'<br>';
				
				//$category_dtls = $common->getCurl($userData,$common->api_url().'products?searchCriteria[filter_groups][0][filters][0][field]=category_id&searchCriteria[filter_groups][0][filters][0][value]='.$subCat->id,'GET',$common->admin_token());	
				//echo count($category_dtls->items).'<br>';
				foreach($category_data[$i]->custom_attributes as $custom_attributes){
					if(1){
						if($custom_attributes->attribute_code == 'include_in_mobile_menu' && $custom_attributes->value == 1){
							foreach($subCat->custom_attributes as $cat_att){							
								if(($cat_att->attribute_code == 'url_key' && $cat_att->value =='home-mobile-app1')){
									...
									//$result[$i]['page_slug'] = 'user-guide1.html';
									$result[$i]['page_slug'] = 'user-guide.html';
									//$result[$i]['slug'] = 'user-guide1';
									$result[$i]['slug'] = 'user-guide';
									$result[$i]['url'] = $common->store_url().'user-guide1.html';
									$result[$i]['menu_id'] = $j;
								} else if($cat_att->attribute_code == 'url_key'  ){ //&& $subCat->children != ''
								...
								if($cat_att->attribute_code=='url_key' && $cat_att->value=='settings'){
									...
									$result[$i]['page_slug'] = 'customer/account/';
								...
								if($cat_att->attribute_code =='url_key' && $cat_att->value =='my-purchases'){
									...
									$result[$i]['page_slug'] = 'sales/order/history/';
								...
								if($cat_att->attribute_code =='url_key' && $cat_att->value =='favourites'){
									...
									$result[$i]['slug'] = 'my-favourites';
									$result[$i]['page_slug'] = 'my-favourites';
									$result[$i]['url'] = $common->store_url().'favourites';
									$result[$i]['menu_id'] = $j;
								}
								if($cat_att->attribute_code =='url_key' && $cat_att->value =='logout'){
									//$result[$i]['slug'] = $cat_att->value;
									$result[$i]['slug'] = 'customer/account/logout/';
									$result[$i]['page_slug'] = 'customer/account/logout/';
		...
		foreach ($result as $index=>$t) {
			if (!isset($t['page_slug'])){
				unset($result[$index]);
			} else {
				$resDuplicate[$i] = $result[$index];
				$i++;
			}
		}
		if($resDuplicate){
			$json = '{ "success" : "1", "data" : '.json_encode($resDuplicate).', "message" : "Menu"}';
		} else {
	...
	public function getCMSContent(CommonController $common) {
		...
		if(trim($page_slug) == 'user-guide.html')
		{
			$page_slug = 'user-guide1.html';
		}
